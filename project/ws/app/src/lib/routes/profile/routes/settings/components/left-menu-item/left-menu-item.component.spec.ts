...
describe('LeftMenuItemComponent', () => {
  ...
  describe('constructor', () => {
    ...
    it('should set language when websiteLanguage exists in localStorage', () => {
      ...
      // Create a new component to test constructor
      TestBed.createComponent(LeftMenuItemComponent)
  ...
  describe('resetPassword', () => {
    it('should call resetPassword service and open link on success', () => {
      const mockResponse = {
        params: { status: 'success' },
        result: { result: { link: 'https://reset-password-link.com' } }
      }
      mockSettingsService.resetPassword.mockReturnValue(of(mockResponse))
      const windowOpenSpy = jest.spyOn(window, 'open').mockImplementation(() => null)

      component.resetPassword()

      expect(mockSettingsService.resetPassword).toHaveBeenCalled()
      expect(windowOpenSpy).toHaveBeenCalledWith('https://reset-password-link.com', '_blank')
    })

    it('should not open link when response status is not success', () => {
      const mockResponse = {
    ...
    it('should not open link when response is null', () => {
      mockSettingsService.resetPassword.mockReturnValue(of(null))
      const windowOpenSpy = jest.spyOn(window, 'open').mockImplementation(() => null)
    ...
    it('should open window even when link is missing in response', () => {
      const mockResponse = {
        params: { status: 'success' },
        result: { result: {} }
      }
  ...
  describe('tourClick - resetPassword', () => {
    it('should call resetPassword when tab name is resetPassword', () => {
      const mockTab = { name: 'resetPassword' }
      jest.spyOn(_, 'camelCase').mockReturnValue('resetPassword')
      jest.spyOn(component, 'resetPassword').mockImplementation(() => { })
    ...
    it('should handle resetPassword with successful response', () => {
      const mockTab = { name: 'resetPassword' }
      const mockResponse = {
        params: { status: 'success' },
        result: { result: { link: 'https://reset-link.com' } }
      }
      mockSettingsService.resetPassword.mockReturnValue(of(mockResponse))
      const windowOpenSpy = jest.spyOn(window, 'open').mockImplementation(() => null)
      ...
      expect(mockSettingsService.resetPassword).toHaveBeenCalled()
      expect(windowOpenSpy).toHaveBeenCalledWith('https://reset-link.com', '_blank')
    })
  })
})
