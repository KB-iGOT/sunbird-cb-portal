...
import {
  hasPermissions,
  hasUnitPermission, NsWidgetResolver, SbUiResolverService
} from '@sunbird-cb/resolver-v2'
import { NetCoreService } from '../services/netcore.service'
import { BtnSettingsService } from '../btn-settings/btn-settings.service'
import { NPSGridService } from '../grid-layout/nps-grid.service'
...
export class InitService {
  ...
  constructor(
    ...
    this.baseUrl = this.configSvc.baseUrl
    this.configSvc.isProduction = this.environment.production
  ...
  async init() {
    ...
    // if (!authenticated) {
    //   this.settingsSvc.initializePrefChanges(this.environment.production)
  ...
  private async fetchUserEnrollDetails(): Promise<NsInstanceConfig.IConfig> {
    const publicConfig: any = await this.enrollSvc.fetchEnrollStats(this.configSvc.userProfile?.userId).toPromise().then((res: any) => {
  ...
  private async fetchFeaturesStatus(): Promise<Set<string>> {
    ...
    const featureConfigs: any = await this.http
      .get<any>(`${this.baseUrl}/features.config.json`)
      .toPromise()
    this.configSvc.restrictedFeatures = new Set(
      Object.entries(featureConfigs)
        .filter(
          ([_k, v]) => !hasPermissions(v as any | undefined, this.configSvc.userRoles, this.configSvc.userGroups),
        )
        .map(([k]) => k),
    )
    return this.configSvc.restrictedFeatures
  }
  private async fetchWidgetStatus(): Promise<any[]> {
    const widgetConfigs = await this.http
      .get<any>(`${this.baseUrl}/widgets.config.json`)
      .toPromise()
  ...
  hasRole(role: string[]): boolean {
    ...
    const rolesForCBP = this.environment.portalRoles
    role.forEach(v => {
