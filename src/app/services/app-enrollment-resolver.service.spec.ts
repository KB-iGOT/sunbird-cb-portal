...
describe('AppEnrollmentResolverService', () => {
  ...
  test('should handle user with no profile', () => {
    ...
    // Verify API call was made with empty userId
    expect(mockHttp.post).toHaveBeenCalledWith(
      `/apis/proxies/v8/learner/course/v4/user/enrollment/details/`,
    //  expect.any(Object)
    );
  });

  test('should handle empty enrollment data array', () => {
    // Mock empty array from dataTransfer
    ...
    expect(result).toEqual({ error: null, data: { courses: [] } });
    expect(mockHttp.post).not.toHaveBeenCalled(); // HTTP call should be skipped
  });
});