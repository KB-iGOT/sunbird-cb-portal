...
describe('AppInterceptorService', () => {
  ...
  describe('intercept', () => {
    describe('when activeOrg and rootOrg are available', () => {
      ...
      it('should not add locale that already exists in lang array', (done) => {
        // 'en' is already in lang (from locale='en-US' → 'en')
        // selectedLangGroup includes 'en' again → should not duplicate
        mockConfigSvc.userPreference = {
        ...
        service.intercept(mockHttpRequest, mockHttpHandler).subscribe(() => {
          const modifiedRequest = mockHttpHandler.handle.mock.calls[0][0] as HttpRequest<any>
          // should still be just 'en', not 'en,en'
          expect(modifiedRequest.headers.get('locale')).toBe('en')
          done()
        })
      })
      ...
      it('should use empty cstoken when cstoken is null', (done) => {
        mockConfigSvc.cstoken = null as any
        const mockResponse = of({} as HttpEvent<any>)
        mockHttpHandler.handle.mockReturnValue(mockResponse)
      ...
      describe('error handling', () => {
        ...
        it('should not handle error status 0 on non-localhost', (done) => {
          // Reset window.location to non-localhost (previous test may have set it to localhost)
          (window as any).location = { origin: 'https://example.com', href: 'https://example.com/page/home' }
          const error = new HttpErrorResponse({
            status: 0,
            statusText: 'Unknown Error',
          })
        ...
        it('should handle error status 419 on non-localhost', (done) => {
          // Reset window.location to non-localhost (previous test may have set it to localhost)
          (window as any).location = { origin: 'https://example.com', href: 'https://example.com/page/home', pathname: '/page/home' }
          // Mock localStorage
          const mockLocalStorage = {
            getItem: jest.fn(),
            removeItem: jest.fn(),
