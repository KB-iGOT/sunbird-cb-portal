...
describe('SignupService', () => {
  ...
  describe('Data Transformation', () => {
    ...
    it('should use empty children when category has no terms (a.terms is undefined)', () => {
      service.list = new Map()
      const frameworkResponse = {
        result: {
          framework: {
            categories: [
              {
                ...
                identifier: 'cat-2',
                index: 2,
                name: 'Category No Terms',
                status: 'active',
                // terms is intentionally absent → a.terms || [] uses []
              }
            ]
          }
        }
      ...
      service.formateData(frameworkResponse)
      expect(service.list.has('cat-no-terms')).toBe(true)
      expect(service.list.get('cat-no-terms').children).toEqual([])
    })
  ...
  describe('API Integration', () => {
    ...
    it('should call getStateOrMinistyForRegistration with correct endpoint', () => {
      const req = { request: { type: 'state' } }
      httpMock.post.mockReturnValueOnce(of({ result: {} }))
      service.getStateOrMinistyForRegistration(req).subscribe()
      expect(httpMock.post).toHaveBeenCalledWith('apis/public/v8/org/hierarchy/search', req)
    })

    it('should call getMinistryForRegistration with correct endpoint', () => {
      const req = { request: { type: 'ministry' } }
      httpMock.post.mockReturnValueOnce(of({ result: {} }))
      service.getMinistryForRegistration(req).subscribe()
      expect(httpMock.post).toHaveBeenCalledWith('apis/public/v8/org/hierarchy/ministry/search', req)
    })

    it('should call getStateForRegistration with correct endpoint', () => {
      const req = { request: { type: 'state' } }
      httpMock.post.mockReturnValueOnce(of({ result: {} }))
      service.getStateForRegistration(req).subscribe()
      expect(httpMock.post).toHaveBeenCalledWith('apis/public/v8/org/hierarchy/state/search', req)
    })

    it('should return You as importedByName when importedById matches empty userId', () => {
      // When additionalProperties.importedById === '' (_.get({}, 'userId', '') === '')
      const terms = [
        {
          code: 'term-you',
          name: 'Term You',
          associations: [],
          additionalProperties: {
            importedById: '',  // matches _.get({}, 'userId', '') which is ''
            importedByName: 'Someone Else',
