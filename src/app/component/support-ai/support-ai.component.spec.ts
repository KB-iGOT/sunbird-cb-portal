...
describe('SupportAIComponent', () => {
  ...
  describe('Localization Methods', () => {
    ...
    it('should return show more text', () => {
      ...
      component.selectedLaguage = 'unknown'
      expect(() => component.showMore()).toThrow()
    })
  })
  ...
  describe('Question and Answer Logic', () => {
    ...
    it('should get user journey filtered by tab', () => {
      component.userJourney = [
        { tab: 'information', type: 'incoming' },
        { tab: 'information', type: 'sendMsg' },
        { tab: 'issue', type: 'incoming' }
      ] as any
  ...
  describe('Priority Questions', () => {
    ...
    it('should handle null responseData', () => {
      ...
      expect(() => component.getPriorityQuestion(1)).toThrow()
    })
  ...
  describe('API Calls Check', () => {
    ...
    it('should check for API calls with existing chat issues', () => {
      ...
      mockLocalStorage.getItem.mockImplementation((key) => {
        if (key === 'selectedLanguage') return 'en'
        if (key === 'faq') return JSON.stringify({
          en: { issue: { quesMap: [], recommendationMap: [], categoryMap: [] } }
        })
        if (key === 'faq-languages') return JSON.stringify([])
        return null
      })
  ...
  describe('Utility Functions', () => {
    ...
    it('should copy video path', () => {
      ...
      // copyPath uses document.createElement and then document.execCommand
    ...
    it('should create initials with full name', () => {
      mockConfigSvc.userProfile.profileImageUrl = null
      mockConfigSvc.userProfile.firstName = 'John Doe'
      component.ngOnInit()
    ...
    it('should create initials with single name', () => {
      mockConfigSvc.userProfile.profileImageUrl = null
      mockConfigSvc.userProfile.firstName = 'John'
      component.ngOnInit()

      // Single name: array = ['John', ''], initials = 'J' + '' = 'J'
      expect(component.initials).toBe('J')
    })

    it('should create initials with undefined in name', () => {
      mockConfigSvc.userProfile.profileImageUrl = null
      mockConfigSvc.userProfile.firstName = 'John undefined'
      component.ngOnInit()

      expect(component.initials).toBe('JU')
    })

    it('should create initials with two-part name', () => {
      mockConfigSvc.userProfile.profileImageUrl = null
      mockConfigSvc.userProfile.firstName = 'John Paul'
      component.ngOnInit()
  ...
  describe('Textarea Management', () => {
    it('should resize textarea with requestAnimationFrame', () => {
      ...
      // After requestAnimationFrame (synchronous mock), height is set to scrollHeight+'px'
      expect(mockTextAreaElement.style.height).toBe('50px')
    ...
    it('should handle missing textArea in reset', () => {
      // nativeElement=null → this.textArea.nativeElement is null → if check is falsy → no throw
      component.textArea = { nativeElement: null } as any
  ...
  describe('Lifecycle Hooks', () => {
    it('should handle ngAfterViewInit', () => {
      ...
      const nativeEl = component.textArea.nativeElement

      component.ngAfterViewInit()

      expect(component.resizeTextarea).toHaveBeenCalledWith(nativeEl, '')
    })
