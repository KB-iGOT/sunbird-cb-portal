...
import { ConfigurationsService, ValueService, NsInstanceConfig } from '@sunbird-cb/utils-v2';
...
describe('AppFooterComponent', () => {
  ...
  it('should set hubsList from instanceConfig if available', async () => {
    ...
    expect(component.hubsList).toEqual([
      { active: true, name: 'Hub1' },
      { active: true, name: 'Hub3' }
    ]);
  });
  ...
  it('should fetch hubsList from API if not in instanceConfig', async () => {
    ...
    expect(component.hubsList).toEqual([{ active: true, name: 'Hub1' }]);
  });

  // it('should set portalUrls from configSvc if available', async () => {
  //   const mockPortalUrls = { portal1: 'url1', portal2: 'url2' };
  //   configServiceMock.portalUrls = mockPortalUrls as any;
